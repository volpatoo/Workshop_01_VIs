...
ortho.Info1<-fieldInfo(mosaic=Veg.Indices,
                     fieldShape=indPlots) ## Extracts all vegetation indices (layers) for each .shp file
...
# Starting parallel
cl <- makeCluster(n.core -3, output = "")
...
system.time(
  for(i in 1:length(imgFiles)){ #loop through images
    ...
    for(v in 1:2){ #loop through two VIs only
